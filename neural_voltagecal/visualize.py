...
loss_list = np.load('conv_loss_list.npy')
plt.plot(loss_list)
plt.show()
#m = input()

Q = scipy.io.loadmat('../data/train/Qout.mat')['Qout']
Q_mean = np.mean(Q,axis=0)
Q_std = np.std(Q,axis=0)
Qn = (Q-Q_mean)/Q_std


V = scipy.io.loadmat('../data/train/V.mat')['V'].T
V_mean = np.mean(V,axis=0)
V_std = np.std(V,axis=0)
Vn = (V-V_mean)/V_std

model = Convnet()
model.load_state_dict(torch.load('../data/model/Convnet_epoch_200_Thu_Jun_14_16:53:30_2018.model'))
